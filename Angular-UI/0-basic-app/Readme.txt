...
2.Add this line "node_modules/bootstrap/dist/css/bootstrap.min.css" into angular.json file inside styles array. (This will update angular.json file with needed changes to reflect bootstrap installation)
3.Copy custom css folder into src inside  angular application. (This folder contains the css used for the html page)
4.Replace the custom html file contents to app.component.html. (This will copy the login page created)
5.If page is loading up with text boxes and buttons but not as expected then , check out if anything is to be added in styles.css file.